...
3. 다음 정보 입력:
   ...
   - **이름**: aria (또는 원하는 앱 이름)
   - **기본 언어**: 한국어
   - **번들 ID**: `aimers.aria` (Xcode와 동일해야 함)
   - **SKU**: 예: `aria-ios-001` (내부 식별용, 고유하면 됨)
4. **만들기** 클릭
...
1. [App Store Connect](https://appstoreconnect.apple.com) → **나의 앱** → **aria** 선택
...
| **서명 오류** | Xcode → 타깃 aria → **Signing & Capabilities**에서 Team 선택, Bundle ID `aimers.aria` 확인 |
...
1. Xcode에서 **aria** 프로젝트 클릭 → **aria** 타깃 선택
...
이 순서대로 하면 TestFlight에서 aria 빌드를 보고, 내부/외부 테스터에게 배포할 수 있습니다.
